...
def calculate_emаs_9_20(closes):
    """
...
def analyze_trade_logic(raw_candles, rr_min=1.5):
    ...
    try:
        ...
        # EMAs (only 9 & 20)
        emas = calculate_emаs_9_20(closes)
        ema9 = emas.get('ema_9')
...
def plot_signal_chart(symbol, raw_candles, signal):
    ...
    if signal.get('entry'): ax1.axhline(signal['entry'], color='blue', linestyle='--', label=f"Entry {fmt_price(signal['entry'])}")
    if signal.get('sl'): ax1.axhline(signal['sl'], color='red', linestyle='--', label=f"SL {fmt_price(signal['sl'])}")
    if signal.get('tp'): ax1.axhline(signal['tp'], color='green', linestyle='--', label=f"TP {fmt_price(signal['tp'])}")
    ax1.legend(loc='upper left', fontsize=9)
    ax1.grid(True, alpha=0.3)

    # volume proxy
    volumes = [abs(closes[i]-opens[i])*1000 for i in range(len(closes))]
    ax2.bar(x, volumes, color=['green' if closes[i] >= opens[i] else 'red' for i in range(len(closes))], alpha=0.6)
