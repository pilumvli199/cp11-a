#!/usr/bin/env python3
# main.py - Enhanced Crypto Trading Bot v5.0 (Rate-limit conservative patch)
...
class SignalDatabase:
    ...
    def save_signal(self, signal_data: Dict):
        ...
        conn.execute("""
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?)
        """, (signal_data['symbol'], signal_data['side'], signal_data['entry'], signal_data['sl'], signal_data['tp'], signal_data['confidence'], signal_data['reason']))
        conn.commit(); conn.close()
    def get_recent_signals_count(self, hours=1) -> int:
        conn = sqlite3.connect(self.db_path)
        cutoff = datetime.now() - timedelta(hours=hours)
        cursor = conn.execute("SELECT COUNT(*) FROM signals WHERE timestamp > ? AND status = 'active'", (cutoff,))
        count = cursor.fetchone()[0]; conn.close(); return count

db = SignalDatabase()

# ---------------- RATE LIMITER (more conservative) ----------------
class RateLimiter:
    """
    Concurrency-safe sliding-window rate limiter with log suppression.
    Conservative defaults to avoid hitting exchange limits.
    """
    def __init__(self, max_requests=5, window_seconds=60, log_suppress_interval=5.0):
...
rate_limiter = RateLimiter(max_requests=5, window_seconds=60, log_suppress_interval=5.0)
...
def detect_pattern(highs: List[float], lows: List[float], closes: List[float]) -> Dict:
    ...
    recent_highs = highs[-10:]; recent_lows = lows[-10:]
...
def horizontal_levels(closes: List[float], highs: List[float], lows: List[float], lookback: int = 50, binsize: float = 0.002) -> List[float]:
    ...
    levels = []
    for point in points:
        if point is None or point == 0: continue
        found=False
        for level in levels:
            try:
...
# ---------------- SAFE ANALYSIS (unchanged logic but defensive) ----------------
def analyze_trade_logic(candles: List[List[float]], rr_min: float = 1.5) -> Dict:
    try:
        ...
        for c in candles:
            ...
            try:
                o=float(c[1]); h=float(c[2]); l=float(c[3]); cl=float(c[4]); filtered.append([o,h,l,cl])
            except Exception:
                continue
        if len(filtered) < 50:
            return {"side": "none", "confidence": 0, "reason": "insufficient valid candle rows after cleaning"}
        closes=[c[3] for c in filtered]; highs=[c[1] for c in filtered]; lows=[c[2] for c in filtered]
        current_price = closes[-1]
        ema_9=ema(closes,9); ema_21=ema(closes,21); ema_50=ema(closes,50)
        rsi_values=calculate_rsi(closes); macd_data=calculate_macd(closes)
        ...
        if current_rsi is not None:
            if current_rsi > 70: confidence+=5; reasons.append(f"overbought RSI({current_rsi:.1f})")
            elif current_rsi < 30: confidence+=5; reasons.append(f"oversold RSI({current_rsi:.1f})")
        try:
            if macd_data["macd"] and macd_data["signal"]:
                macd_current = macd_data["macd"][-1]; signal_current = macd_data["signal"][-1]
                if macd_current > signal_current and len(macd_data["macd"])>1:
                    prev_macd = macd_data["macd"][-2]; prev_signal = macd_data["signal"][-1] if len(macd_data["signal"])>1 else signal_current
                    if prev_macd <= prev_signal: confidence+=10; reasons.append("MACD bullish crossover")
                elif macd_current < signal_current and len(macd_data["macd"])>1:
                    prev_macd = macd_data["macd"][-2]; prev_signal = macd_data["signal"][-1] if len(macd_data["signal"])>1 else signal_current
                    if prev_macd >= prev_signal: confidence+=10; reasons.append("MACD bearish crossover")
        except Exception:
            reasons.append("macd check failed")
        if pattern_info["pattern"] != "none":
            confidence += pattern_info["confidence"] * 0.2; reasons.append(f"pattern: {pattern_info['pattern']}")
        # BUY
        if (current_price is not None and current_ema_9 is not None and current_rsi is not None and support is not None and resistance is not None and confidence >= 60):
            try:
                entry_price = float(current_price)
                stop_loss = float(support * 0.995)
                take_profit = float(resistance * 0.995)
                ...
                if risk_reward >= rr_min:
                    return {"side":"BUY","entry":entry_price,"sl":stop_loss,"tp":take_profit,"confidence":min(95,confidence+10),"reason":"; ".join(reasons),"risk_reward":risk_reward,"indicators":{"rsi":current_rsi,"ema_9":current_ema_9,"ema_21":current_ema_21}}
        ...
        if (current_price is not None and current_ema_9 is not None and current_rsi is not None and support is not None and resistance is not None and confidence >= 60):
            try:
                entry_price = float(current_price)
                stop_loss = float(resistance * 1.005)
                take_profit = float(support * 1.005)
                ...
                if risk_reward >= rr_min:
                    return {"side":"SELL","entry":entry_price,"sl":stop_loss,"tp":take_profit,"confidence":min(95,confidence+10),"reason":"; ".join(reasons),"risk_reward":risk_reward,"indicators":{"rsi":current_rsi,"ema_9":current_ema_9,"ema_21":current_ema_21}}
            except Exception as e:
                reasons.append(f"sell calc error: {e}")
        return {"side":"none","confidence":confidence,"reason":"; ".join(reasons),"indicators":{"rsi":current_rsi or 50,"ema_9":current_ema_9 or current_price,"ema_21":current_ema_21 or current_price}}
    except Exception as ex:
        logger.error(f"analyze_trade_logic fatal: {ex}"); logger.error(traceback.format_exc())
...
def multi_tf_confirmation(c30m: List[List[float]], c1h: List[List[float]], c4h: List[List[float]] = None) -> Dict:
    signal_30m = analyze_trade_logic(c30m); signal_1h = analyze_trade_logic(c1h)
    ...
    if signal_30m["side"] == signal_1h["side"]:
        ...
        if c4h:
            ...
            if signal_4h.get("side") == signal_30m["side"]: confidence_boost += 10
...
def plot_advanced_chart(symbol: str, candles: List[List], signal: Dict, indicators: Dict = None) -> str:
    try:
        ...
        opens=[x[1] for x in good[-50:]]; highs=[x[2] for x in good[-50:]]; lows=[x[3] for x in good[-50:]]; closes=[x[4] for x in good[-50:]]; volumes=[x[5] for x in good[-50:]]
        fig, (ax1, ax2, ax3) = plt.subplots(3,1,figsize=(14,10), gridspec_kw={'height_ratios':[3,1,1]})
        for i,(o,h,l,c) in enumerate(zip(opens,highs,lows,closes)):
            color='green' if c>=o else 'red'
            ax1.plot([i,i],[l,h],color=color,linewidth=0.8); ax1.plot([i-0.3,i+0.3],[o,o],color=color,linewidth=2); ax1.plot([i-0.3,i+0.3],[c,c],color=color,linewidth=2)
        if len(closes)>=21:
            ema9=ema(closes,9); ema21=ema(closes,21)
            if ema9:
                valid=[(i,val) for i,val in enumerate(ema9) if val is not None]
                if valid: x_vals,y_vals=zip(*valid); ax1.plot(x_vals,y_vals,linewidth=1.5,label='EMA9')
            if ema21:
                valid=[(i,val) for i,val in enumerate(ema21) if val is not None]
                if valid: x_vals,y_vals=zip(*valid); ax1.plot(x_vals,y_vals,linewidth=1.5,label='EMA21')
        if signal.get("side")!="none":
            try:
                ...
                pass
        ax1.set_title(f"{symbol} - {signal.get('side','none')} | Conf: {signal.get('confidence',0):.1f}%"); ax1.legend(loc='upper left'); ax1.grid(True,alpha=0.3)
        if len(closes)>=RSI_PERIOD:
            rsi_vals=calculate_rsi(closes)
            if rsi_vals:
                rsi_x=range(len(closes)-len(rsi_vals),len(closes)); ax2.plot(rsi_x,rsi_vals,linewidth=1.2); ax2.axhline(70,linestyle='--'); ax2.axhline(30,linestyle='--'); ax2.set_ylim(0,100)
        ax3.bar(range(len(volumes)), volumes, alpha=0.6)
        plt.tight_layout(); tmp=NamedTemporaryFile(delete=False,suffix=".png"); fig.savefig(tmp.name,bbox_inches='tight',dpi=180); plt.close(fig); return tmp.name
    except Exception as e:
        logger.error(f"plot_advanced_chart error: {e}"); return plot_simple_chart(symbol, candles, signal)
...
async def get_session():
    # lowered connector.limit to reduce concurrent sockets/bursts
...
async def get_enhanced_ai_analysis(symbol: str, candles: List[List], signal: Dict) -> Optional[str]:
    ...
    try:
        ...
        market_summary = {"symbol":symbol,"current_price":current_price,"price_change_24h":f"{price_change_24h:.2f}%","local_signal":{"side":signal.get("side"),"confidence":signal.get("confidence"),"entry":signal.get("entry"),"risk_reward":signal.get("risk_reward")},"indicators":signal.get("indicators",{})}
        system_prompt = "You are an expert crypto trader and technical analyst. Analyze the provided market data and determine if there's a high-confidence trading opportunity. Only suggest signals with 75%+ confidence. Format: SYMBOL - BUY/SELL - ENTRY:price - SL:price - TP:price - CONF:xx% - REASON:brief_explanation"
        user_prompt = f"Market Data Analysis:\n{json.dumps(market_summary, indent=2)}\nPlease provide your independent analysis."
        loop = asyncio.get_running_loop()
        def call_openai():
            return client.chat.completions.create(model=OPENAI_MODEL, messages=[{"role":"system","content":system_prompt},{"role":"user","content":user_prompt}], max_tokens=300, temperature=0.2)
        resp = await loop.run_in_executor(None, call_openai)
        return resp.choices[0].message.content.strip()
    except Exception as e:
        logger.error(f"AI analysis error: {e}"); return None
...
class TelegramNotifier:
    ...
    async def send_message(self, session: aiohttp.ClientSession, text: str, parse_mode: str = "Markdown"):
        if not self.bot_token or not self.chat_id:
            logger.info(f"No Telegram config, log message: {text[:120]}"); return
        url = f"{self.base_url}/sendMessage"; payload = {"chat_id": self.chat_id, "text": text[:4096], "parse_mode": parse_mode}
        try:
            async with session.post(url, json=payload) as r:
                if r.status != 200: logger.error(f"Telegram send message failed: {r.status}")
        except Exception as e:
    ...
    async def send_photo(self, session: aiohttp.ClientSession, caption: str, photo_path: str):
        ...
        try:
            with open(photo_path, 'rb') as photo_file:
                data = aiohttp.FormData(); data.add_field('chat_id', self.chat_id); data.add_field('caption', caption[:1024]); data.add_field('photo', photo_file, filename='chart.png')
                async with session.post(url, data=data) as r:
                    if r.status != 200: logger.error(f"Telegram send photo failed: {r.status}")
        except Exception as e:
...
# ---------------- MARKET SCANNER (stagger increased, semaphore decreased) ----------------
class MarketScanner:
    ...
    async def scan_all_symbols(self, session: aiohttp.ClientSession) -> List[Dict]:
        ...
        if recent_signals >= MAX_SIGNALS_PER_HOUR:
            logger.warning(f"Signal rate limit reached: {recent_signals}/{MAX_SIGNALS_PER_HOUR} per hour"); return []
        tasks=[]
        for symbol in self.symbols:
            tasks.append(asyncio.create_task(self.analyze_symbol(session, symbol)))
            # increased stagger to avoid burst
            await asyncio.sleep(0.25)
        sem = asyncio.Semaphore(2)  # decreased concurrency
        async def limited(t):
            async with sem:
                return await t
        ...
        for i,res in enumerate(results):
            if isinstance(res, Exception):
                logger.error(f"Error analyzing {self.symbols[i]}: {res}"); continue
    ...
    async def analyze_symbol(self, session: aiohttp.ClientSession, symbol: str) -> Optional[Dict]:
        try:
            ...
            if not c30 or not c1:
                logger.warning(f"Failed to fetch candles for {symbol}"); return None
            ...
            if signal.get("side") != "none" and signal.get("confidence",0) >= SIGNAL_CONF_THRESHOLD:
                ...
                if ai_analysis and "NO_SIGNAL" not in ai_analysis.upper():
                    signal["confidence"] = min(100, signal.get("confidence",0) + 5); signal["ai_analysis"] = ai_analysis
                chart_path = plot_advanced_chart(symbol, c30m, signal); signal["chart_path"] = chart_path
                return signal
            return None
        except Exception as e:
...
class PerformanceMonitor:
    def __init__(self): self.start_time=time.time(); self.scan_count=0; self.signal_count=0; self.error_count=0
...
async def process_signal(session: aiohttp.ClientSession, signal: Dict):
    ...
    try:
        ...
        logger.error(f"Failed to process/send signal for {symbol}: {e}"); logger.error(traceback.format_exc())
...
async def enhanced_trading_loop():
    ...
    async with get_session() as session:
        ...
        logger.info("Trading bot started successfully!"); await telegram.send_message(session, startup_msg)
        iteration=0; last_stats=time.time()
        while True:
            iteration+=1; scan_start=time.time()
            try:
                logger.info(f"\n=== SCAN {iteration} @ {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} ==="); monitor.log_scan()
                ...
                logger.info(f"Sleeping for {sleep_time}s until next scan..."); await asyncio.sleep(sleep_time)
            except Exception as e:
                logger.error(f"Critical scan error: {e}"); logger.error(traceback.format_exc())
                await telegram.send_message(session, f"❌ *Scan Error #{iteration}* ` {str(e)[:400]} `"); monitor.log_error()
...
if __name__ == "__main__":
    try:
        ...
        logger.error(f"Fatal error: {e}"); logger.error(traceback.format_exc())
